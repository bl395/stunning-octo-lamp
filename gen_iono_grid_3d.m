...
function [iono_pf_subgrid, iono_pf_subgrid_5, collision_freq_subgrid] = ...
  ...
  for lon = lon_min:lon_inc:lon_max
	...
	hmF2_5 = hmF2_5 .* (1 - fac) + fac.*hmF2_5.*(1 + 0.0015.*(hmF2_5-300));
      end
      ...
      iono_pf_subgrid(lon_idx, :) = chapman(foE, hmE, ymE, foF1, hmF1, ymF1, ...
	                                    foF2, hmF2, ymF2, height_arr);
    ...
    elseif (strcmp(lower(profile_type), 'iri') | ...
      ...
      minute=UT(5);
      minuteselect=UTselect(5,:);
      hourpos=find(hour==hourselect);
...
% script. Otherwise, it will show error as "input(8) arry size...."
 latpos=lat==latitude;
 lonpos=lon==longitude;
iri_options_structure=iri_options(lonpos,latpos,UTfind);
      [iono, iono_extra] = iri2016(lat, lon, R12, UT, ht_min, ht_inc, ...
	                           num_heights, iri_options_structure);
